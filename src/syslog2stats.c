...
int main(
	...
	/* On récupère le nom du programme pour l'aide */
	...
	while(read(fd,&c,1)) {
		...
		if(i==1 && c =='\n') {
			...
			if(tmp_re == NULL) {
				printf("Erreur sur la regexp (%s) de la clé %s\n",valeur,cle);
			} else {
				printf("OK sur la regexp (%s) de la clé %s\n",valeur,cle);
				nb_regexp++;
				if(nb_regexp==1) {
	...
	/* On lit le dernier chunk dont la taille est inférieure à */
	...
	/* On analyse ce chunk pour la présence de caractères de */
	/* retour à la ligne */
	...
	/* On parcourt le reste du fichier à l'envers jusqu'à */
